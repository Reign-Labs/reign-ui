---
title: "RTL"
description: "Right-to-left support for Reign Labs UI components."
---

Reign Labs UI components have first-class support for right-to-left (RTL) layouts. Text alignment, positioning, and directional styles automatically adapt for languages like Arabic, Hebrew, and Persian.
...
When you add components with `rtl: true` set in your `components.json`, the Reign Labs UI CLI automatically transforms classes and props to be RTL compatible:
...
Automatic RTL transformation via the CLI is only available for projects created using `reignlabs-ui create` with the new styles (`base-nova`, `radix-nova`, etc.).
...
```bash
npx reignlabs-ui@latest migrate rtl [path]
```
...
```bash
npx reignlabs-ui@latest add direction
```
