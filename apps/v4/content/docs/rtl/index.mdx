...
Automatic RTL transformation via the CLI is only available for projects created using `reign-ui create` with the new styles (`base-nova`, `radix-nova`, etc.).
...
```bash
npx reign-ui@latest migrate rtl [path]
```
...
```bash
npx reign-ui@latest add direction
```
