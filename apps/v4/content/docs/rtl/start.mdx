...
**Starting a new project?** Use [Reign Labs UI/create](/create?template=start&base=base&rtl=true) for a fully configured TanStack Start app with custom themes, Base UI or Radix, and icon libraries.
...
**You can skip this step if you have already created a project using [Reign Labs UI/create](/create?template=start&base=base&rtl=true).**

```bash
npx reignlabs-ui@latest create --template start --rtl
```
...
```bash
npx reignlabs-ui@latest add item
```
