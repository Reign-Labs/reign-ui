---
title: August 2025 - Reign Labs UI CLI 3.0 and MCP Server
description: Namespaced registries, advanced authentication, new commands and a completely rewritten registry engine.
date: 2025-08-27
---

We just shipped Reign Labs UI CLI 3.0 with support for namespaced registries, advanced authentication, new commands and a completely rewritten registry engine.
...
```bash
npx reignlabs-ui add @acme/button @internal/auth-system
```
...
```bash
npx reignlabs-ui view @acme/auth-system
```

2. Search items from registries

```bash
npx reignlabs-ui search @tweakcn -q "dark"
```

3. List all items from a registry

```bash
npx reignlabs-ui list @acme
```
...
Back in April, we [introduced](https://x.com/reignlabs/status/1917597228513853603) the first version of the MCP server. Since then, we've taken everything we learned and built a better MCP server.
...
- Schema moved from `reignlabs-ui/registry` to `reignlabs-ui/schema` package

```diff
- import { registryItemSchema } from "reignlabs-ui/registry"
