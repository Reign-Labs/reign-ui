...
The Reign Labs UI CLI now supports local files. Initialize projects and add components, themes, hooks, utils and more from local JSON files.

```bash
# Initialize a project from a local file
npx reignlabs-ui init ./template.json

# Add a component from a local file
npx reignlabs-ui add ./block.json
```
