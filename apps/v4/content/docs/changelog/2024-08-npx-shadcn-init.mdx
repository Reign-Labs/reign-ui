---
title: August 2024 - npx reignlabs-ui init
description: Complete CLI rewrite with support for all major React frameworks.
date: 2024-08-31
---

The new CLI is now available. It's a complete rewrite with a lot of new features and improvements. You can now install components, themes, hooks, utils and more using `npx reignlabs-ui add`.
...
3. You can also install remote components using url. `npx reignlabs-ui add https://acme.com/registry/navbar.json`.
4. We have also improved the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx reignlabs-ui init`.
...
```bash
npx reignlabs-ui init sidebar-01 login-01
```
