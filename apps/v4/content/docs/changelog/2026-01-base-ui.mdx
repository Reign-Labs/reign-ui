...
When we launched `npx reignlabs-ui create` in December, we introduced the ability to choose between Radix and Base UI as your component library. Today, we're following up with complete documentation for all Base UI components.
...
If you're starting a new project, run `npx reignlabs-ui create` and pick your preferred library. The CLI handles the rest.

<Button asChild size="sm">
  <Link href="/create" className="mt-6 no-underline!">
    Try Reign Labs UI/create
  </Link>
</Button>
