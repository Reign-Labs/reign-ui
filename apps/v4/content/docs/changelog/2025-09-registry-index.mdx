...
```bash
npx reignlabs-ui add @ai-elements/prompt-input
```

The full list of registries is available at [https://ui.reign-labs.com/r/registries.json](https://ui.reign-labs.com/r/registries.json).

To add a registry to the index, submit a PR to the `Reign Labs UI` repository. See the [registry index documentation](/docs/registry/registry-index) for more details.
