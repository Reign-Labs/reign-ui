...
description: The Reign Labs UI CLI now supports RTL (right-to-left) layouts by automatically converting physical CSS classes to logical equivalents.
date: 2026-01-28
---

Reign Labs UI now has first-class support for right-to-left (RTL) layouts. Your components automatically adapt for languages like Arabic, Hebrew, and Persian.

**This works with the [Reign Labs UI components](/docs/components) as well as any component distributed on the Reign Labs UI registry.**
...
We took a different approach. The Reign Labs UI CLI transforms classes at install time, so you only see logical classes when you actually need them. If you're not building for RTL, you work with familiar classes like `ml-4` and `left-2`. When you enable RTL, the CLI handles the conversion for you.
...
```bash
npx reignlabs-ui@latest init --rtl
```

```bash
npx reignlabs-ui@latest create --rtl
```
...
[![Open in v0](https://v0.app/chat-static/button.svg)](https://v0.app/chat/api/open?url=https://github.com/reign-labs/next-template-rtl)
