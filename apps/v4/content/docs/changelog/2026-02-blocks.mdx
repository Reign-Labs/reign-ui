...
- **Same CLI workflow** - Run `npx reignlabs-ui add` and the CLI will pull the correct block variant based on your project configuration.

If you've already set up your project with `npx reignlabs-ui create`, blocks will automatically use your chosen library. No additional configuration needed.

```bash
npx reignlabs-ui@latest add login-01
```
