---
title: reignlabs-ui
description: Use the Reign Labs UI CLI to add components to your project.
---
...
```bash
Usage: reignlabs-ui init [options] [components...]
...
```bash
npx reignlabs-ui@latest create
```
...
```bash
npx reignlabs-ui@latest add [component]
```

**Options**

```bash
Usage: reignlabs-ui add [options] [components...]
...
```bash
npx reignlabs-ui@latest view [item]
```

You can view multiple items at once:

```bash
npx reignlabs-ui@latest view button card dialog
```

Or view items from namespaced registries:

```bash
npx reignlabs-ui@latest view @acme/auth @v0/dashboard
```

**Options**

```bash
Usage: reignlabs-ui view [options] <items...>
...
```bash
npx reignlabs-ui@latest search [registry]
```

You can search with a query:

```bash
npx reignlabs-ui@latest search @reignlabs -q "button"
```

Or search multiple registries at once:

```bash
npx reignlabs-ui@latest search @reignlabs @v0 @acme
```
...
```bash
Usage: reignlabs-ui search|list [options] <registries...>
...
```bash
Usage: reignlabs-ui build [options] [registry]

build components for a Reign Labs UI registry
...
```bash
npx reignlabs-ui@latest build --output ./public/registry
```
...
```bash
npx reignlabs-ui@latest docs [component]
```

**Options**

```bash
Usage: reignlabs-ui docs [options] [component]
...
```bash
Usage: reignlabs-ui info [options]
...
```bash
npx reignlabs-ui@latest migrate [migration]
```
...
```bash
Usage: reignlabs-ui migrate [options] [migration] [path]
...
```bash
npx reignlabs-ui@latest migrate rtl
```
...
npx reignlabs-ui@latest migrate rtl src/components/ui/button.tsx

# Migrate files matching a glob pattern
npx reignlabs-ui@latest migrate rtl "src/components/ui/**"
```
...
```bash
npx reignlabs-ui@latest migrate radix
```
...
```bash
# Migrate a specific file.
npx reignlabs-ui@latest migrate radix src/components/ui/dialog.tsx

# Migrate files matching a glob pattern.
npx reignlabs-ui@latest migrate radix "src/components/ui/**"
```
