...
```bash
Usage: reign-ui init [options] [components...]
...
```bash
npx reign-ui@latest add [component]
```

**Options**

```bash
Usage: reign-ui add [options] [components...]
...
```bash
npx reign-ui@latest view [item]
```

You can view multiple items at once:

```bash
npx reign-ui@latest view button card dialog
```

Or view items from namespaced registries:

```bash
npx reign-ui@latest view @acme/auth @v0/dashboard
```

**Options**

```bash
Usage: reign-ui view [options] <items...>
...
```bash
npx reign-ui@latest search [registry]
```

You can search with a query:

```bash
npx reign-ui@latest search @reignlabs -q "button"
```

Or search multiple registries at once:

```bash
npx reign-ui@latest search @reignlabs @v0 @acme
```
...
```bash
Usage: reign-ui search|list [options] <registries...>
...
```bash
Usage: reign-ui build [options] [registry]
...
```bash
npx reign-ui@latest build --output ./public/registry
```
...
```bash
npx reign-ui@latest docs [component]
```

**Options**

```bash
Usage: reign-ui docs [options] [component]
...
```bash
npx reign-ui@latest info
```

**Options**

```bash
Usage: reign-ui info [options]
...
```bash
npx reign-ui@latest migrate [migration]
```
...
```bash
Usage: reign-ui migrate [options] [migration] [path]
...
```bash
npx reign-ui@latest migrate rtl
```
...
npx reign-ui@latest migrate rtl src/components/ui/button.tsx

# Migrate files matching a glob pattern
npx reign-ui@latest migrate rtl "src/components/ui/**"
```
...
```bash
npx reign-ui@latest migrate radix
```
...
```bash
# Migrate a specific file.
npx reign-ui@latest migrate radix src/components/ui/dialog.tsx

# Migrate files matching a glob pattern.
npx reign-ui@latest migrate radix "src/components/ui/**"
```
