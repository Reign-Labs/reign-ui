---
title: Tailwind v4
description: How to use Reign Labs UI with Tailwind v4 and React 19.
---
...
One of the major advantages of using `Reign Labs UI` is that the code you end up with is exactly what you'd write yourself. There are no hidden abstractions.
...
```bash
npx reignlabs-ui@latest add --all --overwrite
```
