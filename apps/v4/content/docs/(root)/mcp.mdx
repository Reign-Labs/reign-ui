---
title: MCP Server
description: Use the Reign Labs UI MCP server to browse, search, and install components from registries.
---

The Reign Labs UI MCP Server allows AI assistants to interact with items from registries. You can browse available components, search for specific ones, and install them directly into your project using natural language.

For example, you can ask an AI assistant to "Build a landing page using components from the acme registry" or "Find me a login form from the Reign Labs UI registry".
...
Select your MCP client and follow the instructions to configure the Reign Labs UI MCP server. If you'd like to do it manually, see the [Configuration](#configuration) section.
...
<Tabs defaultValue="claude">
  ...
  <TabsContent value="claude" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx reignlabs-ui@latest mcp init --client claude
       ```

    **Restart Claude Code** and try the following prompts:
       - Show me all available components in the Reign Labs UI registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the Reign Labs UI registry
  ...
  <TabsContent value="cursor" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx reignlabs-ui@latest mcp init --client cursor
       ```

    Open **Cursor Settings** and **Enable the MCP server** for Reign Labs UI. Then try the following prompts:
       - Show me all available components in the Reign Labs UI registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the Reign Labs UI registry
  ...
  <TabsContent value="vscode" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx reignlabs-ui@latest mcp init --client vscode
       ```

    Open `.vscode/mcp.json` and click **Start** next to the Reign Labs UI server. Then try the following prompts with GitHub Copilot:
       - Show me all available components in the Reign Labs UI registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the Reign Labs UI registry
  ...
  <TabsContent value="codex" className="mt-4">
    <Callout className="mt-0">
      **Note:** The `reignlabs-ui` CLI cannot automatically update `~/.codex/config.toml`.
      You'll need to add the configuration manually for Codex.
    </Callout>

    **Run the following command** in your project:
       ```bash
       npx reignlabs-ui@latest mcp init --client codex
       ```

    **Then, add the following configuration** to `~/.codex/config.toml`:
       ```toml
       [mcp_servers.reignlabs-ui]
       command = "npx"
       args = ["reignlabs-ui@latest", "mcp"]
       ```
  ...
  <TabsContent value="opencode" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx reignlabs-ui@latest mcp init --client opencode
       ```
...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io) is an open protocol that enables AI assistants to securely connect to external data sources and tools. With the Reign Labs UI MCP server, your AI assistant gains direct access to:
...
The MCP server acts as a bridge between your AI assistant, component registries and the Reign Labs UI CLI.

1. **Registry Connection** - MCP connects to configured registries (Reign Labs UI, private registries, third-party sources)
...
The Reign Labs UI MCP server works out of the box with any Reign Labs UI-compatible registry.

- **Reign Labs UI Registry** - The default registry with all Reign Labs UI components
- **Third-Party Registries** - Any registry following the Reign Labs UI registry specification
...
You can use any MCP client to interact with the Reign Labs UI MCP server. Here are the instructions for the most popular ones.

### Claude Code

To use the Reign Labs UI MCP server with Claude Code, add the following configuration to your project's `.mcp.json` file:
...
After adding the configuration, restart Claude Code and run `/mcp` to see the Reign Labs UI MCP server in the list. If you see `Connected`, you're good to go.

See the [Claude Code MCP documentation](https://code.claude.com/docs/en/mcp) for more details.

### Cursor

To configure MCP in Cursor, add the Reign Labs UI server to your project's `.cursor/mcp.json` configuration file:
...
After adding the configuration, enable the Reign Labs UI MCP server in Cursor Settings.

Once enabled, you should see a green dot next to the Reign Labs UI server in the MCP server list and a list of available tools.

See the [Cursor MCP documentation](https://docs.cursor.com/en/context/mcp#using-mcp-json) for more details.

### VS Code

To configure MCP in VS Code with GitHub Copilot, add the Reign Labs UI server to your project's `.vscode/mcp.json` configuration file:
...
After adding the configuration, open `.vscode/mcp.json` and click **Start** next to the Reign Labs UI server.
...
To configure MCP in Codex, add the Reign Labs UI server to `~/.codex/config.toml`:
...
<Callout>
  **Note:** No configuration is needed to access the standard Reign Labs UI
  registry.
</Callout>
...
- Show me all available components in the Reign Labs UI registry
- Find me a login form from the Reign Labs UI registry

### Install Items

- Add the button component to my project
- Create a login form using Reign Labs UI components
...
3. **Verify Installation** - Ensure `reignlabs-ui` is installed in your project
...
- [Registry Documentation](/docs/registry) - Complete guide to Reign Labs UI registries
