---
title: Monorepo
description: Using Reign Labs UI components and CLI in a monorepo.
---

Until now, using Reign Labs UI in a monorepo was a bit of a pain. You could add
...
```bash
npx reignlabs-ui@latest init --monorepo
```
...
For example, if you run `npx reignlabs-ui@latest add button`, the CLI will install the button component under `packages/ui` and update the import path for components in `apps/web`.

If you run `npx reignlabs-ui@latest add login-01`, the CLI will install the `button`, `label`, `input` and `card` components under `packages/ui` and the `login-form` component under `apps/web/components`.
