...
On every interaction, the skill runs `reign-ui info --json` to get your project's configuration: framework, Tailwind version, aliases, base library (`radix` or `base`), icon library, installed components, and resolved file paths.
...
2. **Context injection** — It runs `reign-ui info --json` to read your project configuration and injects the result into the assistant's context.
...
4. **Component discovery** — The assistant uses `reign-ui docs`, `reign-ui search`, or MCP tools to find components and their documentation before generating code.
