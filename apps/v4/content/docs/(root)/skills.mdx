---
title: Skills
description: Give your AI assistant deep knowledge of Reign Labs UI components, patterns, and best practices.
---

Skills give AI assistants like Claude Code project-aware context about Reign Labs UI. When installed, your AI assistant knows how to find, install, compose, and customize components using the correct APIs and patterns for your project.
...
This installs the Reign Labs UI skill into your project. Once installed, your AI assistant automatically loads it when working with Reign Labs UI components.
...
On every interaction, the skill runs `reignlabs-ui info --json` to get your project's configuration: framework, Tailwind version, aliases, base library (`radix` or `base`), icon library, installed components, and resolved file paths.
...
Setup and tools for the Reign Labs UI MCP server, which lets AI assistants search, browse, and install components from registries.
...
2. **Context injection** — It runs `reignlabs-ui info --json` to read your project configuration and injects the result into the assistant's context.
3. **Pattern enforcement** — The assistant follows Reign Labs UI composition rules: using `FieldGroup` for forms, `ToggleGroup` for option sets, semantic colors, and correct base-specific APIs.
4. **Component discovery** — The assistant uses `reignlabs-ui docs`, `reignlabs-ui search`, or MCP tools to find components and their documentation before generating code.
