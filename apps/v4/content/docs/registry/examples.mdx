...
### Custom style that extends Reign Labs UI

The following registry item is a custom style that extends Reign Labs UI. On `npx reignlabs-ui init`, it will:
...
The following registry item is a custom style that doesn't extend Reign Labs UI. See the `extends: none` field.

It can be used to create a new style from scratch, i.e. custom components, css vars, dependencies, etc.

On `npx reignlabs-ui add`, the following will:

- Install `tailwind-merge` and `clsx` as dependencies.
- Add the `utils` registry item from the Reign Labs UI registry.
...
The following style will init using Reign Labs UI defaults and then add a custom `brand` color.
...
This block installs the `login-01` block from the Reign Labs UI registry.
...
You can install a block from the Reign Labs UI registry and override the primitives using your custom ones.

On `npx reignlabs-ui add`, the following will:

- Add the `login-01` block from the Reign Labs UI registry.
...
Use `extends: none` to create a base that doesn't extend Reign Labs UI defaults.
