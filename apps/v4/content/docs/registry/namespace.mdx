...
- `@reignlabs/button` - UI component from the Reign Labs UI registry
...
```bash
npx reignlabs-ui@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx reignlabs-ui@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx reignlabs-ui@latest add @acme/button
```
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx reignlabs-ui@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx reignlabs-ui@latest view @acme/button
```
...
```json title="registry-item.json" showLineNumbers
{
  ...
  "registryDependencies": [
    "@reignlabs/card", // From default registry
...
When you run `npx reignlabs-ui@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx reignlabs-ui@latest add @acme/auth @custom/login-form
```
...
```bash
npx reignlabs-ui@latest add @my-company/custom-button
```
...
```json title="dashboard.json" showLineNumbers
{
  "name": "dashboard",
  "registryDependencies": [
    "@reignlabs/card", // 1. Resolved first
    "@vendor/chart", // 2. Resolved second
    "@custom/card" // 3. Resolved last (overrides @reignlabs/card)
  ]
}
```
...
1. `@reignlabs/card` - installs to `components/ui/card.tsx`
...
The Reign Labs UI CLI provides several commands for working with namespaced registries:
...
npx reignlabs-ui@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx reignlabs-ui@latest add https://registry.example.com/button.json

# Install from local file
npx reignlabs-ui@latest add ./local-registry/button.json
```
...
npx reignlabs-ui@latest view @v0/dashboard @reignlabs/card

# View from URL
npx reignlabs-ui@latest view https://registry.example.com/button.json
```
...
npx reignlabs-ui@latest search @v0

# Search with query
npx reignlabs-ui@latest search @acme --query "auth"

# Search multiple registries
npx reignlabs-ui@latest search @v0 @acme @lib

# Limit results
npx reignlabs-ui@latest search @v0 --limit 10 --offset 20

# List all items (alias for search)
npx reignlabs-ui@latest list @acme
```
...
```bash
npx reignlabs-ui@latest add @non-existent/component
```
