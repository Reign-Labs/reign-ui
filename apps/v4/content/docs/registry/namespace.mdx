...
```bash
npx reign-ui@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx reign-ui@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx reign-ui@latest add @acme/button
```
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx reign-ui@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx reign-ui@latest view @acme/button
```
...
When you run `npx reign-ui@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx reign-ui@latest add @acme/auth @custom/login-form
```
...
```bash
npx reign-ui@latest add @my-company/custom-button
```
...
npx reign-ui@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx reign-ui@latest add https://registry.example.com/button.json

# Install from local file
npx reign-ui@latest add ./local-registry/button.json
```
...
npx reign-ui@latest view @v0/dashboard @reignlabs/card

# View from URL
npx reign-ui@latest view https://registry.example.com/button.json
```
...
npx reign-ui@latest search @v0

# Search with query
npx reign-ui@latest search @acme --query "auth"

# Search multiple registries
npx reign-ui@latest search @v0 @acme @lib

# Limit results
npx reign-ui@latest search @v0 --limit 10 --offset 20

# List all items (alias for search)
npx reign-ui@latest list @acme
```
...
```bash
npx reign-ui@latest add @non-existent/component
```
