...
The `reign-ui` CLI will automatically generate this file for you when you run the `build` command.
...
You can change the output directory by passing the `--output` option. See the [reign-ui build command](/docs/cli#build) for more information.
...
To install a registry item using the `reign-ui` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx reign-ui@latest add http://localhost:3000/r/hello-world.json
```
