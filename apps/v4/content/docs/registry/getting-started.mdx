...
If you're starting a new registry project, you can use the [registry template](https://github.com/reign-labs/registry-template) as a starting point. We have already configured it for you.
...
If you'd like to see an example of a registry, we have a [template project](https://github.com/reign-labs/registry-template) for you to use as a starting point.
...
The `reignlabs-ui` CLI will automatically generate this file for you when you run the `build` command.
...
{
  "$schema": "https://ui.reign-labs.com/schema/registry.json",
  "name": "acme",
...
### Install the Reign Labs UI CLI

```bash
npm install reignlabs-ui@latest
```
...
{
  "scripts": {
    "registry:build": "reignlabs-ui build"
  }
}
```
...
You can change the output directory by passing the `--output` option. See the [reignlabs-ui build command](/docs/cli#build) for more information.
...
To install a registry item using the `reignlabs-ui` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx reignlabs-ui@latest add http://localhost:3000/r/hello-world.json
```
