...
describe("defaultScaffold", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    delete process.env.REIGNLABS_TEMPLATE_DIR
    delete process.env.REIGNLABS_GITHUB_URL
    mockExit = vi
      .spyOn(process, "exit")
  ...
  it("should use local templates when REIGNLABS_TEMPLATE_DIR is set", async () => {
    process.env.REIGNLABS_TEMPLATE_DIR = "/local/templates"
