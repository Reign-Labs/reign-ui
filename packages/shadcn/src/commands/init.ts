...
import { BUILTIN_REGISTRIES, REIGNLABS_URL } from "@/src/registry/constants"
        ...
        if (options.template === "laravel" && !hasPackageJson) {
          ...
          logger.log(
            `  See ${highlighter.info(
              `${REIGNLABS_URL}/docs/installation/laravel`
      ...
      if (options.preset !== undefined) {
        ...
        if (typeof presetArg === "string") {
          ...
          if (isUrl(presetArg)) {
            ...
            if (url.pathname === "/init" && presetArg.startsWith(REIGNLABS_URL)) {
