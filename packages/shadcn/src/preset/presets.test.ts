...
const REIGNLABS_URL = REGISTRY_URL.replace(/\/r\/?$/, "")
...
describe("createPresetUrl", () => {
  it("should not include rtl by default", () => {
    ...
    expect(parsed.origin + parsed.pathname).toBe(`${REIGNLABS_URL}/create`)
...
describe("buildInitUrl", () => {
  it("should build url with all preset fields as query params", () => {
    ...
    const parsed = new URL(url)
    expect(parsed.origin + parsed.pathname).toBe(`${REIGNLABS_URL}/init`)
