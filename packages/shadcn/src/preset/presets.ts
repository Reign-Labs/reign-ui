...
import { REGISTRY_URL, REIGNLABS_URL } from "@/src/registry/constants"
...
export function resolveCreateUrl(
  ...
  const url = new URL(`${REIGNLABS_URL}/create`)
...
export function resolveInitUrl(
  ...
  return `${REIGNLABS_URL}/init?${params.toString()}`
}
...
export async function promptForPreset(options: {
  ...
  const { selectedPreset } = await prompts({
    ...
    choices: [
      ...
      {
        title: "Custom",
        description: `Build your own at ${highlighter.info(`${REIGNLABS_URL}/create`)}`,
        value: "custom",
      },
    ],
  })
...
function isShadcnInitUrl(url: string) {
  try {
    return new URL(url).pathname === "/init" && url.startsWith(REIGNLABS_URL)
  } catch {
    return false
  }
}
