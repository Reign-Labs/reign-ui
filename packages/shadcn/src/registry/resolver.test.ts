...
describe("resolveRegistryItemsFromRegistries", () => {
  ...
  it("should resolve non-registry items through @reignlabs registry", () => {
    const items = ["button", "card", "dialog"]
    const config = { registries: {} } as any

    const result = resolveRegistryItemsFromRegistries(items, config)

    // Non-prefixed items are resolved through the built-in @reignlabs registry
    expect(result).toEqual([
      "https://ui.reign-labs.com/r/styles/{style}/button.json",
      "https://ui.reign-labs.com/r/styles/{style}/card.json",
      "https://ui.reign-labs.com/r/styles/{style}/dialog.json",
    ])
    expect(setRegistryHeaders).toHaveBeenCalledWith({})
  })
  ...
  it("should handle mixed registry and non-registry items", () => {
    ...
    // Non-registry items (button, dialog) are resolved through the built-in @reignlabs registry
    expect(result).toEqual([
      "https://ui.reign-labs.com/r/styles/{style}/button.json",
      "https://v0.dev/chat/b/card/json",
      "https://ui.reign-labs.com/r/styles/{style}/dialog.json",
