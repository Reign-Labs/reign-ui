...
describe("checkNamespaceConfiguration", () => {
  ...
  it("should handle built-in registries as configured", () => {
    const result = checkNamespaceConfiguration(["@reignlabs", "@foo"], mockConfig)

    expect(result.configured).toEqual(["@reignlabs", "@foo"])
    expect(result.missing).toEqual([])
  })
  ...
  it("should handle mixed configured and missing namespaces", () => {
    const result = checkNamespaceConfiguration(
      ["@reignlabs", "@foo", "@unknown", "@bar", "@missing"],
      mockConfig
    )

    expect(result.configured).toContain("@reignlabs")
