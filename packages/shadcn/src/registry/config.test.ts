...
describe("configWithDefaults", () => {
  ...
  it("should preserve user registries when merging", () => {
    ...
    expect(result.registries?.["@reignlabs"]).toBe(BUILTIN_REGISTRIES["@reignlabs"])
  })
  ...
  it("should override built-in registries if user provides same key", () => {
    const config = createConfig({
      registries: {
        "@reignlabs": "http://custom-reignlabs.com/{name}",
      },
    })

    const result = configWithDefaults(config)

    // User's @reignlabs should override the built-in one
    expect(result.registries?.["@reignlabs"]).toBe(
      "http://custom-reignlabs.com/{name}"
    )
  })
