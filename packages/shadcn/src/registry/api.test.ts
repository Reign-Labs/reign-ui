...
describe("getRegistryItem", () => {
  it("should read and parse a valid local JSON file", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tmpdir(), "reignlabs-test-"))
  ...
  it("should handle tilde (~) home directory paths", async () => {
    ...
    const tempFile = path.join(homeDir, "reignlabs-test-tilde.json")
    ...
    try {
      ...
      const tildeePath = "~/reignlabs-test-tilde.json"
...
describe("getRegistry", () => {
  ...
  it("should distinguish between URL and registry name", async () => {
    ...
    const registryName = "@reignlabs"
    const registryUrl = "https://ui.reign-labs.com/registry.json"

    // Mock for URL
    server.use(
      http.get(registryUrl, () => {
        return HttpResponse.json({
          name: "reignlabs-from-url",
    ...
    const mockConfig = {
      ...
      registries: {
        "@reignlabs": {
          url: "https://ui.reign-labs.com/{name}.json",
        },
      },
    } as any
    ...
    expect(urlResult.name).toBe("reignlabs-from-url")
...
describe("getRegistriesConfig", () => {
  ...
  it("should handle complex registry configurations", async () => {
    ...
    const config = {
      style: "new-york",
      registries: {
        "@reignlabs": "https://ui.reign-labs.com/r/styles/{style}/{name}.json",
    ...
    try {
      ...
      expect(result.registries?.["@reignlabs"]).toBe(
        "https://ui.reign-labs.com/r/styles/{style}/{name}.json"
      )
