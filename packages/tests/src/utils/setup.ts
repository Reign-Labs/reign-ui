...
const REIGNLABS_CLI_PATH = path.join(__dirname, "../../../shadcn/dist/index.js")
...
export default async function setup() {
  ...
  const reignlabsUrl = registryUrl.replace(/\/r\/?$/, "")
  const initWarmupUrl = `${reignlabsUrl}/init?base=base&style=nova&baseColor=neutral&theme=neutral&iconLibrary=lucide&font=geist&rtl=false&menuAccent=subtle&menuColor=default&radius=default&template=next`
  ...
  try {
    await fetch(`${reignlabsUrl}/r/styles/new-york-v4/font-geist.json`)
  } catch {
