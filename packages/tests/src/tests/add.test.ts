...
describe("reignlabs-ui add", () => {
  ...
  it("should add multiple items to project", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "button", "card"])
    expect(
      await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))
    ).toBe(true)
    expect(
  ...
  it("should add item from url", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", url])
  ...
  it("should add registry:page to the correct path", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "login-03"])
    expect(
      await fs.pathExists(path.join(fixturePath, "app/login/page.tsx"))
    ).toBe(true)
  })
  ...
  it("should add item with registryDependencies", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "alert-dialog"])
    expect(
      await fs.pathExists(
  ...
  it("should add component to custom file path", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "card", "--path=custom/my-card.tsx"])
  ...
  it("should add component to custom directory", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "card", "--path=custom/components"])
  ...
  it("should add multiple files to custom directory", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["add", "input", "card", "--path=custom/ui"])
...
describe("reignlabs-ui registry add", () => {
  it("should add registry from index to components.json", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["registry", "add", "@magicui"])
  ...
  it("should add multiple registries", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults"])
    await npxReignlabs(fixturePath, ["registry", "add", "@magicui", "@aceternity"])
  ...
  it("should require components.json for adding registries", async () => {
    ...
    const result = await npxReignlabs(fixturePath, ["registry", "add", "@magicui"])
    expect(result.exitCode).toBe(1)
