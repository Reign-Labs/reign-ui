...
import { createFixtureTestDirectory, npxReignlabs } from "../utils/helpers"
...
describe("reignlabs-ui view", () => {
  it("should view a single component from reignlabs registry", async () => {
    ...
    await configureRegistries(fixturePath, {
      "@reignlabs": "http://localhost:9080/r/{name}",
    })
    const output = await npxReignlabs(fixturePath, ["view", "button"])
  ...
  it("should view multiple components from reignlabs registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "button", "card"])
  ...
  it("should view component with registry dependencies", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "alert-dialog"])
  ...
  it("should view component from configured registry", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@one/foo"])
  ...
  it("should view component with nested registry dependencies", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@one/bar"])
  ...
  it("should view component with cross-registry dependencies", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@one/complex"])
  ...
  it("should handle authentication for secured registries", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@two/secure-item"])
  ...
  it("should handle non-existent component gracefully", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "non-existent"])
  ...
  it("should handle non-existent registry gracefully", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "@unknown/component"])

    expect(output.stdout).toContain('Unknown registry "@unknown"')
  })

  it("should work with @reignlabs namespace", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, [
      "view",
      "@reignlabs/button",
  ...
  it("should handle invalid URL format", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    const output = await npxReignlabs(fixturePath, ["view", "not-a-valid-url"])
  ...
  it("should handle missing environment variables for authenticated registry", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@auth/secure-item"])
  ...
  it("should view authenticated dependencies when directly requested", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@two/item"])
  ...
  it("should handle namespace with special characters", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "@test-123/component"])
  ...
  it("should handle empty component name in namespace", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "@reignlabs/"])
  ...
  it("should handle double namespace", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, ["view", "@@test/component"])
  ...
  it("should error when viewing non-existent item from configured registry", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["view", "@one/does-not-exist"])
