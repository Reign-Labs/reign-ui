...
describe("reignlabs-ui init - next-app", () => {
  ...
  it("should init without CSS variables", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults", "--no-css-variables"])
  ...
  it("should init with components", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults", "button"])
...
describe("reignlabs-ui init - custom style", async () => {
  ...
  it("should init with style that extends reignlabs", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "http://localhost:4445/r/style.json"])
...
describe("reignlabs-ui init - template flag", () => {
  ...
  it("should accept valid template with --defaults", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "-t", "next", "--defaults"])
...
describe("reignlabs-ui init - --name flag", () => {
  ...
  beforeAll(async () => {
    testBaseDir = path.join(os.tmpdir(), `reignlabs-name-test-${process.pid}`)
  ...
  it("should create a new project with the specified name", async () => {
    ...
    await npxReignlabs(emptyDir, ["init", "--defaults", "--name", projectName], {
      timeout: 120000,
    })
...
describe("reignlabs-ui init - next-monorepo", () => {
  ...
  beforeAll(async () => {
    testBaseDir = path.join(os.tmpdir(), `reignlabs-monorepo-test-${process.pid}`)
    await fs.ensureDir(testBaseDir)
  })
  ...
  it("should create a monorepo with preset", async () => {
    ...
    const result = await npxReignlabs(
      testBaseDir,
      [
        "init",
        "--name",
...
describe("reignlabs-ui init - rtl flags", () => {
  ...
  it("should set rtl to false with --no-rtl", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app")
    await npxReignlabs(fixturePath, ["init", "--defaults", "--no-rtl"])
...
describe("reignlabs-ui init - existing components.json", () => {
  ...
  it.skip("should override existing components.json when using --force", async () => {
    ...
    // Reinit with --force.
    await npxReignlabs(fixturePath, ["init", "--force", "--defaults"])
