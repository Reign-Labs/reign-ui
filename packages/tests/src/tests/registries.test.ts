...
const registryReignlabs = await createRegistryServer(
  [
    {
      name: "utils",
      ...
      files: [
        {
          ...
          content:
            "export function AlertDialog() {\n  return <div>AlertDialog Component from Registry Reignlabs</div>\n}",
          type: "registry:ui",
        },
        {
          path: "components/ui/button.tsx",
          content:
            "export function Button() {\n  return <div>Button Component from Registry Reignlabs</div>\n}",
          type: "registry:ui",
        },
      ],
    },
    ...
    },
    {
      name: "example-button",
      type: "registry:component",
      registryDependencies: ["@reignlabs/button"],
      files: [
        {
          path: "components/example-button.tsx",
          content:
            "export function ExampleButton() {\n  return <div>Example Button Component from Registry Reignlabs</div>\n}",
          type: "registry:component",
        },
      ],
    },
...
describe("registries", () => {
  it("should add from registry using url", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "http://localhost:4444/r/foo"])
  ...
  it("should add multiple items from registry using urls", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    await npxReignlabs(fixturePath, [
      "add",
  ...
  it("should show only built-in registries when not configured", async () => {
    ...
    // Should show registries since @reignlabs is built-in
    expect(output.stdout).toContain("registries:")
    expect(output.stdout).toContain("@reignlabs")
  ...
  it("should show an error when adding from a non-existent registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    const output = await npxReignlabs(fixturePath, [
      "add",
  ...
  it("should show an error when adding from a non-existent registry with configured registries", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["add", "@acme/component"])
  ...
  it("should add item using namespaced registry", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["add", "@one/foo"])
  ...
  it("should add multiple items using namespaced registry", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/foo", "@one/bar"])
  ...
  it("should add registry dependencies when adding from namespaced registry", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/baz"])
  ...
  it("should add nested registry dependencies when adding from namespaced registry", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/qux"])
  ...
  it("should show an error when adding namespaced with unconfigured registry", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["add", "@two/two"])
    expect(output.stdout).toContain('Unknown registry "@one"')
  })
  ...
  it("should show an error when authentication is not configured", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["add", "@two/one"])
  ...
  it("should add item when authentication is configured", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@two/one"])
  ...
  it("should show an error for nested unauthorized dependencies", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["add", "@one/quux"])
    expect(output.stdout).toContain("Unauthorized")
  })
  ...
  it("should add item when authentication is configured and nested dependencies are authorized", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/quux"])
  ...
  it("should expand env vars in registries config", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/foo"])
    expect(
      await fs.pathExists(path.join(fixturePath, "components/foo.tsx"))
    ).toBe(true)
  })
  ...
  it("should expand env vars in registries config with multiple registries", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "@one/foo", "@three/baz", "@two/two"])
  ...
  describe("fetchFromRegistry improvements", () => {
    it("should handle registry resolution transparently", async () => {
      ...
      await npxReignlabs(fixturePath, ["add", "@one/foo", "@two/two"])
    ...
    it("should maintain type safety with mixed registries", async () => {
      ...
      await npxReignlabs(fixturePath, ["add", "@one/foo", "button"])
    ...
    it("should handle circular dependencies gracefully", async () => {
      ...
      await npxReignlabs(fixturePath, ["add", "@circular/comp-a"])
    ...
    it("should preserve header context across dependency resolution", async () => {
      ...
      await npxReignlabs(fixturePath, ["add", "@auth/parent"])
  ...
  describe("built-in registries", () => {
    it("should error when trying to override @reignlabs in config", async () => {
      const fixturePath = await createFixtureTestDirectory("next-app-init")
      ...
      // Add @reignlabs as a registry
      config.registries = {
        "@reignlabs": "https://my-custom-registry.com/{name}",
      }
    ...
    it("should work with @reignlabs namespace without configuration", async () => {
      const fixturePath = await createFixtureTestDirectory("next-app-init")

      // Don't configure any registries - @reignlabs should be built-in
      const output = await npxReignlabs(fixturePath, ["add", "@reignlabs/button"])
  ...
  it("should add registry:item with no framework", async () => {
    ...
    await npxReignlabs(fixturePath, ["add", "no-framework-item"])
  ...
  it("should add named item from reignlabs registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    await npxReignlabs(fixturePath, ["add", "button"])
    expect(
      await fs.pathExists(path.join(fixturePath, "components/ui/button.tsx"))
    ).toBe(true)
  })

  it("should add namespaced items from the reignlabs registry", async () => {
    const fixturePath = await createFixtureTestDirectory("next-app-init")
    await npxReignlabs(fixturePath, [
      "add",
      "@reignlabs/example-button",
      "@reignlabs/no-framework-item",
    ])
    expect(
      await fs.pathExists(
  ...
  it("should add namespaced items from reignlabs registry with no-framework", async () => {
    const fixturePath = await createFixtureTestDirectory("no-framework")
    await npxReignlabs(fixturePath, ["add", "@reignlabs/no-framework-item"])
    expect(await fs.pathExists(path.join(fixturePath, "path/to/foo.txt"))).toBe(
      true
    )
    expect(
...
describe("registries:init", () => {
  ...
  it("should error when init with unconfigured registries", async () => {
    ...
    const output = await npxReignlabs(fixturePath, ["init", "@two/style"])
